---
name: pkg-auth-util-coding-standards-and-best-practices
description: 'Use after carecard-must-do. Mandatory for every pkg-auth-util task, including analysis, clarification, planning, implementation, review, debugging, documentation, public API work, skill maintenance, and validation. Use before every narrower skill.'
---
...
After `carecard-must-do`, load this skill before doing any work in `pkg-auth-util`, including read-only
...
- `$pkg-auth-util-auth-crypto-library` for cryptography, package architecture,
...
1. Read the complete request and inspect implementation, public exports,
   declarations, tests, documentation, and consumers before deciding how to
   change the package.
2. Translate the request into a coherent technical contract. Do not apply
   ...
   incompatible with cryptographic or package architecture.
...
4. Ask for clarification when an unresolved choice would materially change a
   public API, cryptographic behavior, security, consumer behavior, destructive
   scope, or the repositories that must change.
5. Explain architectural tradeoffs before a major API, type, crypto, module,
...
- Prefer readable direct implementation over clever compression or a temporary
  workaround.
- Use meaningful names that describe authentication and cryptographic intent.

## Function Evolution

Before changing a function's behavior or signature, inventory every direct,
indirect, test, exported, callback, configuration-driven, and dynamic consumer.
...
- Treat every exported, declared, public, callback, or dynamically discovered
  function as shared unless single use is conclusively proven.
...
overloads, duplicate exports, dual paths, transitional names, or fallback
...
- Keep crypto parsing, validation, transformation, signing, and error
  responsibilities explicit rather than hiding them in one long function.
